...
4. Build the XKit extension from source with `gulp build:PLATFORM`, where `PLATFORM` is one of the supported platforms.  See [`gulp build`](#gulp-build) for more information.
5. Reload the XKit extension in the browser under test:
...
6. Open the XKit settings menu and navigate to Other > Update All and click "Update all my extensions".
