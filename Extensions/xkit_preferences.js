...
XKit.extensions.lang_english.xkit_preferences = new Object({
	...
	language_panel: {
		...
		"text": "More comfortable in another language than English? Click on the Get Extensions tab below to download a language "+
		"file. After that, you can select the language you like here."
	}
...
XKit.extensions.xkit_preferences = new Object({
	...
	run: function() {
		...
		$("body").bind("keydown keyup", function(event) {
			...
			if(event.altKey) {
				...
				if (!XKit.extensions.xkit_preferences.showing_help_button) {
	...
	news: {
		...
		unread_count: function() {
			...
			for (i=0;i<prev_objects.length;i++) {
				...
				if (!prev_objects[i].read) {
					if (typeof prev_objects[i].important !== "undefined") {
						if (!show_all && prev_objects[i].important !== "1") {
							continue;
						}
					}
					m_return++;
		...
		create: function(id, title, message, date, important) {
			...
			if (XKit.extensions.xkit_preferences.news.check(id)) {
    ...
    function startFestivusInterval() {
      ...
      XKit.extensions.xkit_preferences.festivus_lights_interval = setInterval(function() {
		...
		if (XKit.extensions.xkit_preferences.hide_xcloud_if_not_installed) {
		...
		if (XKit.extensions.xkit_preferences.bubble_tour_mode) {
		...
		XKit.download.page("gallery.php", function(mdata) {
			...
			if (mdata.server_down) {
				...
				$("#xkit-extensions-panel-right-inner").html(
					'<div class="xkit-unable-to-load-extension-gallery">' + XKit.lang.get("xkit_preferences.gallery.error") + "</div>");
				$("#xkit-extensions-panel-right").nanoScroller();
				$("#xkit-extensions-panel-right").nanoScroller({ scroll: 'top' });
				return;
			}
	...
	gallery_parse_item: function(obj) {
		...
		if (XKit.installed.check(obj.name)) { return ""; }
	...
	show_my_extensions: function(iconic) {
		...
		if (typeof iconic === "undefined") {
			...
			if (iconic === "false" || !iconic) { iconic = false; }
			if (iconic === "true" || iconic) { iconic = true; }
		} else {
			if (iconic === "false" || !iconic) { XKit.storage.set("xkit_preferences","list_type","false"); }
			if (iconic === "true" || iconic) { XKit.storage.set("xkit_preferences","list_type","true"); }
		}
	...
	fill_extensions: function(internal, iconic) {
		...
		for (i=0;i<installed.length;i++) {
			...
			if (!internal && installed[i].substring(0,5) === "xkit_") {
				continue;
			}
	...
	open_extension_control_panel: function(extension_id) {
		...
		if (XKit.extensions[extension_id].slow) {
		...
		$("#xkit-extension-reset").click(function() {
			...
			XKit.window.show("Reset " + m_ext.title + "?",
				...
				'<div id="xkit-close-message" class="xkit-button">' + XKit.lang.get("common.cancel") + '</div>');
		...
		$("#xkit-extension-enabled").click(function() {
			...
			if (XKit.installed.enabled(m_ext)) {
	...
	return_extension_settings: function(extension_id) {
		...
		for(var pref in XKit.extensions[extension_id].preferences) {
			...
			if (XKit.extensions[extension_id].preferences[pref].type === "blog") {
				...
				var m_extra_classes = "";
				if (XKit.extensions[extension_id].preferences[pref].experimental || XKit.extensions[extension_id].preferences[pref].slow) {
				...
				if (XKit.extensions[extension_id].preferences[pref].experimental) {
					m_return = m_return + '<div class="xkit-extension-experimental-bong">&nbsp;</div>';
				} else if (XKit.extensions[extension_id].preferences[pref].slow) {
		...
		var m_html =
				'<div class="nano long" id="xkit-extensions-panel-left">' +
					'<div class="content" id="xkit-extensions-panel-left-inner">' +
						...
						'<div data-pname="config" class="xkit-extension text-only">'+ XKit.lang.get("xkit_preferences.other.config_data") + "</div>" +
