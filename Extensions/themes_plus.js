			...
			var
				...
				setSelector = function(hsb, cal) {
					...
					$(cal).data('colorpicker').selectorIndic.css({
						left: Math.floor(150 * parseInt(hsb.s) / 100),
						top: Math.floor(150 * (100 - parseInt(hsb.b)) / 100)
					});
				},
				setHue = function(hsb, cal) {
					$(cal).data('colorpicker').hue.css('top', Math.floor(150 - 150 * parseInt(hsb.h) / 360));
				},
				...
				downIncrement = function(ev) {
					...
					var current = {
						...
						y: ev.pageY,
						field: field,
						val: parseInt(field.val().toString(), 10),
				...
				moveHue = function(ev) {
					...
					ev.data.cal.data('colorpicker')
						.fields
						.eq(4)
						.val(Math.floor(360 * (150 - Math.max(0, Math.min(150, (ev.pageY - ev.data.y)))) / 150))
						.get(0),
					[ev.data.preview]
				);
					return false;
				...
				moveSelector = function(ev) {
					...
					ev.data.cal.data('colorpicker')
						.fields
						.eq(6)
						.val(Math.floor(100 * (150 - Math.max(0, Math.min(150, (ev.pageY - ev.data.pos.top)))) / 150))
						.end()
						.eq(5)
						.val(Math.floor(100 * (Math.max(0, Math.min(150, (ev.pageX - ev.data.pos.left)))) / 150))
						.get(0),
					[ev.data.preview]
				);
					return false;
				...
				init: function(opt) {
					...
					return this.each(function() {
						if (!$(this).data('colorpickerId')) {
							...
							var id = 'collorpicker_' + Math.floor(Math.random() * 1000);
		...
		$("#xkit-themes-plus-import-export").click(function() {
			...
			$("#xkit-themes-plus-import-confirm").click(function() {
				...
				$("#xkit-themes-plus-confirm-input-code").click(function() {
					...
					var data = $data.val().toString().trim();
